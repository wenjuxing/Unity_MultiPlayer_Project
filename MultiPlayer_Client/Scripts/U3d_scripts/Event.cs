...
namespace Kaiyun
{
    ...
    public class Event
    {
        ...
        public static void UnregisterIn(string eventName, object target, string methodName)
        {
            ...
            {
                List<GsHandler> list ;
                if (eventInDict.TryGetValue(eventName, out List<GsHandler> value)) list = value;
                else list = null;
        ...
        public static void UnregisterOut(string eventName, object target, string methodName)
        {
            ...
            {
                List<GsHandler> list;
                if (eventOutDict.TryGetValue(eventName, out List<GsHandler> value)) list = value;
                else list = null;
        ...
        { 
            if (System.Threading.Thread.CurrentThread.ManagedThreadId == 1)
            {
                ...
                while (outQueue.Count > 0)
                {
                    ...
                    List<GsHandler> list;
                    if (eventOutDict.TryGetValue(item.name, out List<GsHandler> value)) list = value;
                    else list = null;
